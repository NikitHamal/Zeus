...
public class IO {
	...
	public static final byte[] readSome(File path, int limit)
		...
		try (SilentFileInputStream in = new SilentFileInputStream(path)) {
			return readNBytesCompat(in, limit);
		}
	}
	...
	public static final byte[] readFully(File path, int max)
			throws FileNotFoundException, IOException {
		try (SilentFileInputStream in = new SilentFileInputStream(path)) {
			byte[] buf = readNBytesCompat(in, max);
			if (in.read() != -1) {
	...
	public static ByteBuffer readWholeStream(InputStream in, int sizeHint)
			throws IOException {
		return ByteBuffer.wrap(readAllBytesCompat(in, sizeHint));
	}
	...
	public static void readFully(final InputStream fd, final byte[] dst,
		...
		int read = readNBytesCompat(fd, dst, off, len);
		if (read != len)
	...
	public static int readFully(InputStream fd, byte[] dst, int off)
			throws IOException {
		return readNBytesCompat(fd, dst, off, dst.length - off);
	}
	...
	// Android runtime compat: InputStream.readNBytes/readAllBytes only exist on
	// Android API 33+ (Java 9+). These helpers run on any runtime.
	...
	private static int readNBytesCompat(InputStream in, byte[] dst, int off,
			int len) throws IOException {
		int total = 0;
		while (total < len) {
			int n = in.read(dst, off + total, len - total);
			if (n < 0) {
				break;
			}
			total += n;
	...
	private static byte[] readAllBytesCompat(InputStream in, int sizeHint)
			throws IOException {
		ByteArrayOutputStream out = sizeHint > 0
				? new ByteArrayOutputStream(sizeHint)
				: new ByteArrayOutputStream();
		byte[] tmp = new byte[8192];
		int n;
		while ((n = in.read(tmp, 0, tmp.length)) >= 0) {
			out.write(tmp, 0, n);
		}
		return out.toByteArray();
	}
