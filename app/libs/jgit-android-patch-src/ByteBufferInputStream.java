...
public class ByteBufferInputStream extends InputStream {
	...
	public int read(byte[] b, int off, int len) throws IOException {
		nullCheck();
		// Android runtime compat: Objects.checkFromIndexSize only exists on
		// Android API 33+ (Java 9+). Inline the bounds check.
		if (off < 0 || len < 0 || len > b.length - off) {
			throw new IndexOutOfBoundsException(
					"off=" + off + ", len=" + len + ", size=" + b.length); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		}
		if (len == 0) {
			return 0;
		}
